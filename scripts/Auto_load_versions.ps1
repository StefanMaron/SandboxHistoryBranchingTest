...
$Versions | Sort-Object -Property Country, Version | % {
    ...
    if ($LastCommit.Length -eq 0) {
        ...
        $BranchAlreadyExists = ((git branch --list -r "origin/$($country)-$($Version.Major)-$($Version.Build)") -ne $null) -or ((git branch --list "$($country)-$($Version.Major)-$($Version.Build)") -ne $null)

        if ($BranchAlreadyExists) {
            git switch "$($country)-$($Version.Major)-$($Version.Build)"
        }
        else {
            if ($CommitIDLastCUFromPreviousMajor -ne $null) {
                git switch -c "$($country)-$($Version.Major)-$($Version.Build)" $CommitIDLastCUFromPreviousMajor
            }
            else {
                git switch -c "$($country)-$($Version.Major)-$($Version.Build)" $LatestCommitIDOfBranchEmpty                
            }
        }
        ...
        git gc | out-null
        git pull origin
        git push --set-upstream origin "$($country)-$($Version.Major)-$($Version.Build)"
