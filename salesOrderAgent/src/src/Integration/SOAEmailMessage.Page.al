...
page 4404 "SOA Email Message"
{
    ...
    begin
        ...
        GlobalPreviousMessageText := SOAImpl.GetPreviousText(Rec);
        PreviousMessagesVisible := GlobalPreviousMessageText <> '';
        IsMessageEditable := AgentMessage.IsEditable(Rec);
        AttachmentCount := SOAImpl.GetNumberOfAttachments(Rec);
        ...
        if Rec.Type = Rec.Type::Output then begin
            ...
            SOAImpl.GetSentMessageToAddress(Rec, GlobalSendToAddress);
    ...
    begin
        ...
        if ContactVisible then
            CustomerVisible := FindCustomer(GlobalContact, GlobalCustomer);
    ...
    local procedure FindCustomer(var Contact: Record Contact; var Customer: Record Customer): Boolean
    var
        SOAFiltersImpl: Codeunit "SOA Filters Impl.";
    begin
        exit(SOAFiltersImpl.FindCustomer(Contact, Customer));
    end;
