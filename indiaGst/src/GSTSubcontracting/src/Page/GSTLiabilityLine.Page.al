...
page 18472 "GST Liability Line"
{
    ...
    layout
    {
        ...
        area(Factboxes)
        {
            ...
            {
                ApplicationArea = Basic, Suite;
                SubPageLink = "Table ID Filter" = const(18470),
                    "Document No. Filter" = field("Liability Document No."),
                    "Line No. Filter" = field("Liability Document Line No.");
            }
        }
    }
        ...
        area(Processing)
        {
            group("&Line")
            {
                ...
                action("&Dimensions")
                {
                    Caption = '&Dimensions';
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the line dimensions.';
                    Image = Dimensions;
                    Promoted = true;
                    PromotedOnly = true;
            ...
            group("P&osting")
            {
                Caption = 'P&osting';
                Image = Post;
                ToolTip = 'Click Post to Finalize the document or journal by posting the amounts and quantities to the related accounts in your company books';
                ...
                action("P&ost")
                {
                    ...
                    var
                        ...
                        PostJnlQst: label 'Do you want to post the journal lines?';
                    begin
                        if not confirm(PostJnlQst) then
                            exit;

                        SubcontractingPostGSTLiab.PostGSTLiability(Rec);
                        Rec.DeleteAll();
                        CurrPage.Close();
                    end;
                }
